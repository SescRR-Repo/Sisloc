// ViewModels/MotoristaEditViewModel.cs
...
{
    /// <summary>
    /// ViewModel para edição de motoristas.
    /// </summary>
    public class MotoristaEditViewModel
    {
        ...
        // Propriedades para validação de documentação
        ...
        public bool DocumentacaoVencida => DiasParaVencimentoCnh < 0 || DiasParaVencimentoExameTox < 0;
        ...
        public string GetClasseStatusDocumentacao()
        {
            if (DiasParaVencimentoCnh < 0 || DiasParaVencimentoExameTox < 0)
                return "danger";
            if (DiasParaVencimentoCnh <= 30 || DiasParaVencimentoExameTox <= 60)
        ...
        public List<string> GetAlertasDocumentacao()
        {
            ...
            else if (DiasParaVencimentoCnh <= 30)
                alertas.Add($"CNH vence em {DiasParaVencimentoCnh} dias");

            if (DiasParaVencimentoExameTox < 0)
                alertas.Add("Exame toxicológico vencido!");
            else if (DiasParaVencimentoExameTox <= 60)
                alertas.Add($"Exame toxicológico vence em {DiasParaVencimentoExameTox} dias");

            return alertas;
        }

        // Restrições para edição
        public bool PodeAlterarCnh => !TemAgendamentosAtivos;
        public bool PodeAlterarCategoria => !TemAgendamentosAtivos;
    }
}